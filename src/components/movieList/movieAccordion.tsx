...
const MovieAccordion = ({ info }: any) => {
  ...
  const [startingPanel, setStartingPanel] = useState(0);
  const [passedPanels, setPassedPanels] = useState(0);
  ...
  const swipeLeft = (x: number) => {
    setPassedPanels(Math.floor(x / 350));
    setOpenCard(
      startingPanel - passedPanels < 9 ? startingPanel - passedPanels : 9
    );
  };

  const swipeRight = (x: number) => {
    setPassedPanels(Math.floor(x / 350));
    setOpenCard(
      startingPanel - passedPanels > 0 ? startingPanel - passedPanels : 0
    );
  };

  const mobileSwipeLeft = (x: number) => {
    setPassedPanels(Math.floor(x / 200));
    setOpenCard(
      startingPanel - passedPanels < 9 ? startingPanel - passedPanels : 9
    );
  };

  const mobileSwipeRight = (x: number) => {
    setPassedPanels(Math.floor(x / 200));
    setOpenCard(
      startingPanel - passedPanels > 0 ? startingPanel - passedPanels : 0
    );
    ...
    <>
      <div>
        <Swipe
          ...
          onSwipingLeft={(delta: number) => {
            swipeLeft(delta);
          }}
          onSwipingRight={(delta: number) => {
            swipeRight(delta);
          }}
          onSwipeEnd={() => {
            setPassedPanels(0);
            setStartingPanel(openCard);
          }}
        >
                  ...
                  >
                    <Card className="d-flex rounded-0 bg-dark movie-accordion-card flex-row border-0 text-white">
                      ...
                      <Card.Img
                        ...
                        src={
                          'http://localhost:3000/images/movie_placeholder.jpg'
                        }
                      />
                    </Card>
                    <div
                    ...
                    >
                      <h4 className="accordion-score">
                        {info[idx].vote_average}
                      </h4>
                    </div>
                  </div>
                );
        ...
        <Swipe
          nodeName="div"
          className="mobile-accordion-container accordion-container"
          onSwipingLeft={(delta: number) => {
            mobileSwipeLeft(delta);
          }}
          onSwipingRight={(delta: number) => {
            mobileSwipeRight(delta);
          }}
          onSwipeEnd={() => {
            setPassedPanels(0);
            setStartingPanel(openCard);
