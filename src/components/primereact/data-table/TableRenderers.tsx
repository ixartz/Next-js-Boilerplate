...
export const renderPrice = (
  rowData: Game,
  renderFunc: (rowData: Game) => string
) => {
  const price = renderFunc(rowData)
  return price !== undefined ? price : '-'
}

export const renderHomePrice = (rowData: Game) => {
  const outcome = rowData?.bookmakers[0]?.markets[0]?.outcomes[0]
  return outcome ? outcome.price.toString() : '-'
}

export const renderAwayPrice = (rowData: Game) => {
  const outcome = rowData?.bookmakers[0]?.markets[0]?.outcomes[1]
  return outcome ? outcome.price.toString() : '-'
}

export const renderDrawPrice = (rowData: Game) => {
  const outcome = rowData?.bookmakers[0]?.markets[0]?.outcomes[2]
  return outcome ? outcome.price.toString() : '-'
}
